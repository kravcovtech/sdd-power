---
name: sdd-power
description: "Дисциплина разработки по существующей документации с персистентным трекингом (PLAN.md, WORKLOG.md, TIMELINE.md, LESSONS.md в docs/sdd-power/) и критической проверкой документации на ошибки. Используй ВСЕГДА, когда пользователь говорит «начни разработку по документации», «продолжи разработку», «где мы остановились», «что сделано, что осталось», «сверься с планом», «обнови план и лог», «зафиксируй прогресс», «продолжи с места остановки», «веди план/таймлайн разработки»; упоминает PLAN.md, WORKLOG.md, TIMELINE.md, LESSONS.md или директорию docs/sdd-power; жалуется, что документация проекта содержит ошибки или расходится с кодом; или начинает многошаговую разработку, которая растянется на несколько сессий. Триггерься и без явного упоминания трекинга, если в проекте есть docs/sdd-power или файлы PLAN.md и WORKLOG.md (в т.ч. в корне — след старой версии скилла). НЕ используй для разовых мелких правок, укладывающихся в одну сессию."
---
...
0. **Приведи инфраструктуру к текущей версии скилла.** Проект мог быть инициализирован более ранней версией, где части системы ещё не существовало. Проверь и почини молча всё недостающее (шаг идемпотентен — в актуальном проекте он ничего не делает):
   - файлы трекинга лежат в корне проекта или в другом месте — перенеси в `docs/sdd-power/`;
   - нет `docs/sdd-power/stages/` — создай;
   - нет строки-указателя в CLAUDE.md — добавь (без неё скилл не подхватится в следующей сессии — это главный якорь);
   - нет записи `docs/sdd-power/` в `.gitignore` при наличии git — добавь;
   - в файлах нет полей текущих шаблонов (например, «Коммит», «Проверено», «DoD этапа») — не переписывай старые записи, просто веди новые по актуальному шаблону.
   Что-то починил — одна запись в WORKLOG о миграции, со списком.
